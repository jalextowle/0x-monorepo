...
// TODO(jalextowle): It would be much better not to need to use the any type; however,
// the yargs Argument type is fairly opaque
export async function abiGenAsync(params: any): Promise<void> {
    const mainTemplate = utils.getNamedContent(params.template);
    const template = Handlebars.compile<ContextData>(mainTemplate.content);
    const abiFileNames = globSync(params.abis);
    const partialTemplateFileNames = globSync(params.partials);

    if (params.language === 'TypeScript') {
        registerTypeScriptHelpers(params.backend);
    } else if (params.language === 'Python') {
        registerPythonHelpers();
    }
    registerPartials(partialTemplateFileNames);
    ...
    for (const abiFileName of abiFileNames) {
        ...
        const outFilePath = (() => {
            if (params.language === 'TypeScript') {
                return `${params.output}/${outFileName}.ts`;
            } else if (params.language === 'Python') {
                const directory = `${params.output}/${outFileName}`;
                mkdirp.sync(directory);
                return `${directory}/__init__.py`;
            } else {
                throw new Error(`Unexpected language '${params.language}'`);
            }
        })();

        if (utils.isOutputFileUpToDate(outFilePath, [abiFileName, params.template, ...partialTemplateFileNames])) {
        ...
        const methodsData = _.map(methodAbis, (methodAbi, methodAbiIndex: number) => {
            ...
            const languageSpecificName: string = makeLanguageSpecificName(
                params.language,
                sanitizedMethodAbis[methodAbiIndex].name,
            );
        ...
        const eventsData = _.map(eventAbis, (eventAbi, eventAbiIndex: number) => {
            const languageSpecificName = makeLanguageSpecificName(params.language, eventAbi.name);
...
function registerPartials(partialTemplateFileNames: string[]): void {
...
function registerTypeScriptHelpers(backend: string): void {
    Handlebars.registerHelper('parameterType', utils.solTypeToTsType.bind(utils, ParamKind.Input, backend));
    Handlebars.registerHelper('assertionType', utils.solTypeToAssertion.bind(utils));
    Handlebars.registerHelper('returnType', utils.solTypeToTsType.bind(utils, ParamKind.Output, backend));
...
function makeLanguageSpecificName(language: string, methodName: string): string {
    return language === 'Python' ? changeCase.snake(methodName) : methodName;
}
